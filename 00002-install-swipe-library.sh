...
wget -O "${BUILD_DIR}/custom/libjni_latinimegoogle.so" "${IME_LIBRARY}"
...
sed -i "\$aPRODUCT_COPY_FILES += custom/libjni_latinimegoogle.so:system/lib64/libjni_latinimegoogle.so" ${BUILD_DIR}/build/make/target/product/core.mk
